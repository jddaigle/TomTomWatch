...
import net.studioblueplanet.strava.Message;
import net.studioblueplanet.strava.MessageObservable;
import net.studioblueplanet.strava.SettingsDialog;
import net.studioblueplanet.strava.StravaUploadProcess;
...
/**
 * View for the TomTomWatch application. Supports Multisport, Spark/Runner and Adventurer
 *
 * @author Jorgen
 */
public class TomTomWatchView extends javax.swing.JFrame implements Observer
{
	 ...
	 *            The process for communication to the watch
	 */
	...
	public TomTomWatchView(CommunicationProcess communicationProcess)
	{
		...
		getContentPane().setMinimumSize(new Dimension(1300, 1000));
		...
		jListActivities.setModel(model);
		jListActivities.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);

		// Initialize the listbox
		model = new DefaultListModel<>();
		jListRoutes.setModel(model);
		model.addElement("Press Load to download routes, Add/Delete and Save the result to the watch.");
		jListRoutes.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);

		// Set the autosave option
		jCheckBoxAutoSave.setSelected(settings.getBooleanValue("autoSaveTtbin"));

		// Initialize the map
		jPanelMap.setLayout(new BoxLayout(jPanelMap, BoxLayout.Y_AXIS));

		if (settings.getStringValue("mapService").equals("google"))
		{
			map = new MapGoogle(jPanelMap);
		}
		else if (settings.getStringValue("mapService").equals("osm"))
		{
			map = new MapOsm(jPanelMap);
		}
		else
		{
			map = null;
		...
		{
			jMenuDebugging.setVisible(false);
		}
		...
		jCheckBoxSmooth.setSelected(trackSmoothing);
		...
		jCheckBoxCompress.setSelected(trackCompression);

		// If simulation data is used, disable the menu item to save simulation data
		if (settings.getBooleanValue("simulation"))
		{
			jMenuItemSaveSimSet.setEnabled(false);
		}
		jPanel3.setLayout(new MigLayout("", "[grow][grow][grow][grow][grow][grow]", "[17px][grow][grow][grow][grow][grow][grow]"));
		jPanel3.add(jLabel7, "cell 0 0 2 1,alignx left,aligny top");

		jButtonStrava = new JButton();
		jButtonStrava.addActionListener(new ActionListener()
			{
				@Override
				public void actionPerformed(ActionEvent e)
				{
					MessageObservable test = new MessageObservable();
					test.addObserver(getJFrame());

					setStatus("Uploading to Strava...");
					ActivityData data = communicationProcess.getActivityData(jListActivities.getSelectedIndex());
					if (data == null)
					{
						appendStatus("There is no activity selected.");
					}
					else
					{
						appendStatus("Uploading [" + jListActivities.getSelectedValue().toString() + "]");
						StravaUploadProcess.uploadToStrava(data, test);
					}
				}
			});

		jButtonStrava.setText("Strava");
		jButtonStrava.setFont(new Font("Lucida Sans", Font.PLAIN, 12));
		jPanel3.add(jButtonStrava, "cell 4 1,growx");
		jPanel3.add(jProgressBar, "cell 0 6 4 1,grow");
		jPanel3.add(jPanel2, "cell 0 3 4 3,grow");
		jPanel2.setLayout(new MigLayout("", "[grow][grow][grow][grow]", "[grow][grow]"));
		jPanel2.add(jRadioButtonRunning, "cell 0 0,grow");
		jPanel2.add(jRadioButtonCycling, "cell 0 1,growx,aligny center");
		jPanel2.add(jRadioButtonSwimming, "cell 1 1,alignx left,growy");
		jPanel2.add(jRadioButtonHiking, "cell 1 0,grow");
		jPanel2.add(jRadioButtonSkating, "cell 2 1,grow");
		jPanel2.add(jRadioButtonDriving, "cell 2 0,grow");
		jPanel2.add(jRadioButtonMulti, "cell 3 1,grow");
		jPanel2.add(jRadioButtonFlying, "cell 3 0,grow");
		jPanel3.add(jCheckBoxDownloadMostRecent, "cell 4 3 2 1,alignx left,aligny bottom");
		jPanel3.add(jCheckBoxAutoSave, "cell 4 4 2 1,alignx left,aligny top");
		jPanel3.add(jCheckBoxSmooth, "cell 4 5,alignx left,aligny top");
		jPanel3.add(jCheckBoxCompress, "cell 4 6 2 1,alignx left,aligny top");
		jPanel3.add(jLabel2, "cell 0 2,alignx left,aligny center");
		jPanel3.add(jTextFieldGpxFile, "cell 1 2 4 1,growx,aligny top");
		jPanel3.add(jButtonSaveGpx, "cell 5 2,growx,aligny top");
		jPanel3.add(jButtonDownload, "cell 0 1,growx");
		jPanel3.add(jButtonErase, "cell 1 1,growx");
		jPanel3.add(jButtonUploadGps, "cell 2 1,growx");
		jPanel3.add(jButtonLoadTtbin, "cell 3 1,growx");
		jPanel3.add(jButtonSaveTtbin, "cell 5 1,growx");

		getContentPane().setLayout(new MigLayout("", "[117px][grow][117px][grow][117px][grow][117px][grow][117px][grow]", "[23px][350:n,grow][197px][146px]"));
		getContentPane().add(jScrollPane1, "cell 0 3 10 1,grow");
		getContentPane().add(jPanel3, "cell 0 2 4 1,growx,aligny top");
		getContentPane().add(jScrollPane2, "cell 0 1 4 1,grow");
		getContentPane().add(jLabel4, "cell 0 0,alignx right,aligny center");
		getContentPane().add(jTextFieldWatch, "cell 1 0,growx,aligny top");
		getContentPane().add(jLabel11, "cell 2 0,alignx right,aligny center");
		getContentPane().add(jTextFieldSerial, "cell 3 0,growx,aligny top");
		getContentPane().add(jLabel8, "cell 4 0,alignx right,aligny center");
		getContentPane().add(jTextFieldProductId, "cell 5 0,growx,aligny top");
		getContentPane().add(jLabel10, "cell 6 0,alignx right,aligny center");
		getContentPane().add(jTextFieldTime, "cell 7 0,growx,aligny top");
		getContentPane().add(jLabel9, "cell 8 0,alignx right,aligny center");
		getContentPane().add(jTextFieldFirmware, "cell 9 0,growx,aligny top");
		getContentPane().add(jPanelMap, "cell 4 1 6 1,grow");
		getContentPane().add(jPanel5, "cell 4 2 6 1,grow");
		jPanel5.setLayout(new MigLayout("", "[grow][grow][grow][grow][grow][grow]", "[17px][23px][24px][97px]"));

		jTextFieldRouteGpx = new JTextField();
		jTextFieldRouteGpx.addActionListener(new ActionListener()
			{
				@Override
				public void actionPerformed(ActionEvent e)
				{}
			});
		jPanel5.add(jTextFieldRouteGpx, "cell 1 1 3 1,grow");
		jTextFieldRouteGpx.setColumns(10);
		jPanel5.add(jScrollPane3, "cell 0 3 6 1,grow");
		jPanel5.add(jLabel6, "cell 0 0,alignx left,aligny top");
		jPanel5.add(jLabel5, "cell 0 2,alignx right,aligny center");
		jPanel5.add(jLabel3, "cell 0 1,alignx right,aligny center");
		jPanel5.add(jTextFieldRouteName, "cell 1 2,grow");
		jPanel5.add(jButtonAddRoute, "cell 2 2,growx,aligny bottom");
		jPanel5.add(jButtonDeleteRoute, "cell 3 2,growx,aligny bottom");
		jPanel5.add(jButtonChooseRoute, "cell 4 1,growx,aligny top");
		jPanel5.add(jButtonDeleteAllRoutes, "cell 4 2,growx,aligny bottom");
		jPanel5.add(jButtonSaveRoutes, "cell 5 2,growx,aligny bottom");
		jPanel5.add(jButtonListRoutes, "cell 5 1,growx,aligny top");
	}

	/**
	 * This method redefines the fonts on the UI. It replaces the fonts by fonts incorporated in the application.
	 */
	public void setFont()
	{
		Font monospace11pt;
	...
	private void initComponents()
	{
		...
		jMenuItemSecrets = new JMenuItem("Strava Secrets");

		final JFrame frame = this;
		jMenuItemSecrets.addActionListener(new ActionListener()
			{
				@Override
				public void actionPerformed(ActionEvent e)
				{
		...
		jMenuFile.add(jMenuItemSecrets);
	...
	{// GEN-HEADEREND:event_jButtonSaveGpxActionPerformed
		...
		String fileName = fileChooser(jTextFieldGpxFile.getText(), path, "Save", "GPX files (*.gpx)", "gpx");
	...
	{// GEN-HEADEREND:event_jButtonSaveTtbinActionPerformed
		...
		index = jListActivities.getSelectedIndex();
		...
		if (data != null)
		{
			...
			if (fileName != null)
			{
				fileName = fileChooser(fileName, null, "Save", "TTBIN files (*.ttbin)", "ttbin");
				if (fileName != null)
				{
					// Write the file
					...
					if (!fileSaveError)
					{
						...
						if (!fileSaveError)
						{
							...
							model = (DefaultListModel<String>) jListActivities.getModel();
	...
	{// GEN-HEADEREND:event_jButtonLoadTtbinActionPerformed
		...
		fileName = fileChooser(ttbinPath, null, "Load", "TTBIN files (*.ttbin)", "ttbin");
	...
	/**
	 * This method handles the menu item requesting saving a watch file to disk
	 */
	...
	{// GEN-HEADEREND:event_jButtonChooseRouteActionPerformed
		...
		fileName = jTextFieldRouteGpx.getText();
		path = settings.getStringValue("routeFilePath");

		fileName = fileChooser(fileName, path, "Load", "GPX files (*.gpx)", "gpx");

		if (fileName != null)
		{
			jTextFieldRouteGpx.setColumns(5); // prevents textfield from resizing...
			jTextFieldRouteGpx.setText(fileName);
			...
			name = name.replace(".GPX", "");
			jTextFieldRouteName.setText(name);
			...
			jListActivities.clearSelection();
	...
	{// GEN-HEADEREND:event_jButtonAddRouteActionPerformed
		...
		if (communicationProcess.isConnected())
		{
			name = jTextFieldRouteName.getText().trim();
			file = jTextFieldRouteGpx.getText().trim();
			...
			if (!name.equals(""))
			{
				if (name.length() <= MAXNAME)
				{
					if (!file.equals(""))
					{
						index = jListRoutes.getSelectedIndex();
						if (index >= 0)
						{
	...
	{// GEN-HEADEREND:event_jMenuItemListHistoryActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_LISTHISTORY);
	...
	{// GEN-HEADEREND:event_jMenuItemUpdateFirmwareActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_UPDATEFIRMWARE);
	...
	{// GEN-HEADEREND:event_jMenuItemSaveSimSetActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_SAVESIMULATIONSET);
	...
	{// GEN-HEADEREND:event_jMenuItemListRacesActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_LISTRACES);
	...
	{// GEN-HEADEREND:event_jMenuItemShowTrackedActivityActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_LISTTRACKEDACTIVITY);
	...
	{// GEN-HEADEREND:event_jMenuItemUploadFileActionPerformed
		...
		if (communicationProcess.isConnected())
		{
			...
			fileName = fileChooser(fileName, null, "Upload", "bin (*.bin)", "bin");
			if (fileName != null)
			{
	...
	{// GEN-HEADEREND:event_jMenuItemShowSettingsActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_SHOWWATCHSETTINGS);
	...
	{// GEN-HEADEREND:event_jMenuItemSyncTimeActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_SYNCTIME);
	...
	{// GEN-HEADEREND:event_jMenuItemDeletePreferencesActionPerformed
		...
		if (communicationProcess.isConnected())
		{
			response = JOptionPane.showConfirmDialog(null, "Do you want to delete the preference file from the watch? Connectivity data will be lost",
					"Confirm",
					JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
			if (response == JOptionPane.YES_OPTION)
			{
	...
	{// GEN-HEADEREND:event_jButtonDeleteRouteActionPerformed
		...
		{
			appendStatus("First select route to delete\n");
		}
	...
	{// GEN-HEADEREND:event_jMenuItemShowWorkoutsActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_LISTWORKOUTS);
	...
	{// GEN-HEADEREND:event_jMenuItemWorkoutListActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_LISTWORKOUTSUMMARY);
	...
	{// GEN-HEADEREND:event_jMenuItemDownloadWorkoutsActionPerformed
		checkAndPushCommand(ThreadCommand.THREADCOMMAND_DOWNLOADWORKOUTS);
	...
	{// GEN-HEADEREND:event_jMenuItemUploadWorkoutsActionPerformed
		...
		if (communicationProcess.isConnected())
		{
			...
			fileName = fileChooser(fileName, null, "Upload", "json (*.json)", "json");
			if (fileName != null)
			{
	...
	/**
	 * This method pushes a command on the command stack. First however a check is made whether a watch is connected
	 *
	 * @param command
	 *            The command to push.
	 */
	...
	/**
	 * Returns the activity description to be shown in the list box. Format is: [v] - [start datetime] [id] - [type] [activity] [v] - 'v': File has been saved,
	 * 'x': file has not been saved [start datetime] - Start of the activity [id] - 0x99999999, file ID [type] - 'file' for loaded from file, 'watch' for
	 * downloaded from watch [activity] - the activity: 'run', 'swim' etc
	 *
	 * @param data
	 *            Data defining the activity
	 * @param prefix
	 *            Prefix indicating the source of the activity ('file' or 'watch')
	...
	/**
	 * Based on the information in the activity, set the corresponding radio button
	 *
	 * @param activity
	 *            The activity for which the radio button is set
	 */
	...
	/**
	 * Generate the output file name based on the activity and selected activity type (radio button)
	 *
	 * @param activity
	 *            The activity to base the filename on
	 */
	private void generateGpxFileName(Activity activity)
	{
		...
		if (jRadioButtonRunning.isSelected())
		{
			activityPrefix = "running";
		}
		else if (jRadioButtonCycling.isSelected())
		{
			activityPrefix = "cycling";
		}
		else if (jRadioButtonHiking.isSelected())
		{
			activityPrefix = "hiking";
		}
		else if (jRadioButtonSwimming.isSelected())
		{
			activityPrefix = "swimming";
		}
		else if (jRadioButtonDriving.isSelected())
		{
			activityPrefix = "driving";
		}
		else if (jRadioButtonFlying.isSelected())
		{
			activityPrefix = "flying";
		}
		else if (jRadioButtonSkating.isSelected())
		{
		...
		currentFileName = jTextFieldGpxFile.getText();
		...
		if (currentFileName.equals(""))
		{
			...
			jTextFieldGpxFile.setText(fileName);
		}
		else
		{
	 ...
	 *
	 * @param fileNameOnly
	 *            True if only the GPX filename needs to be updated
	 */
	...
	{
		return jCheckBoxDownloadMostRecent.isSelected();
	}
	...
	public void clear()
	{
		...
		// Clear the list
		model = (DefaultListModel) jListActivities.getModel();
		model.clear();
	...
	public void clearRoutes()
	{
		...
		// Clear the list
		model = (DefaultListModel) jListRoutes.getModel();
		model.clear();
	...
	{
		jTextFieldTime.setText(time.format("hh:mm:ss"));
		return false;
	}
	 ...
	 *
	 * @param promile
	 *            Progress in the range of 0-1000
	 */
	public void setProgress(int promile)
	{
		jProgressBar.setValue(promile);
	}
	...
	public TomTomWatchView getJFrame()
	{
		return this;
	}
	...
	{
		return jCheckBoxAutoSave.isSelected();
	}
	 ...
	 *            The item to add
	 * @param prefix
	 *            Prefix indicating the source of the data: 'file' or 'watch'
	 */
	@SuppressWarnings("unchecked")
	...
	{
		jTextFieldWatch.setText(deviceName);
	}
	 ...
	 *
	 * @param version
	 *            String defining the firmware
	 */
	public void setFirmwareVersion(String version)
	{
		jTextFieldFirmware.setText(version);
	}
	...
	{
		jTextFieldSerial.setText(serial);
	}
	 ...
	 *
	 * @param routes
	 *            Routes to display in the listbox
	 * @param index
	 *            Index to highlight
	 */
	...
	{
		jButtonDeleteAllRoutes.setEnabled(enabled);
		jButtonDeleteRoute.setEnabled(enabled);
		jButtonAddRoute.setEnabled(enabled);
		jButtonSaveRoutes.setEnabled(enabled);
		jButtonListRoutes.setEnabled(enabled);
	}
	...
	public void update(Observable o, Object arg)
	{
		if (arg instanceof Message)
		{
			...
			{
				case APPEND:
					appendStatus(message.getMessage());
					break;
				case SET:
	...
	private JButton jButtonStrava;
	private JMenuItem jMenuItemSecrets;
	private JTextField jTextFieldRouteGpx;
