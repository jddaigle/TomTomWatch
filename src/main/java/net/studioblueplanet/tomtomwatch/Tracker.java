...
/**
 * This class reads and stores the tracked activities of the watch. This data is stored in the 0x00B1nnnn files and contains steps, calories, rest heartrate,
 * etc
 *
 * @author Jorgen
 */
public class Tracker
{
	...
	public class TrackedDataRecord
	{
		...
		public void add(TrackedDataRecord record)
		{
			...
			measurementInterval += record.measurementInterval;
			steps += record.steps;
			active += record.active;
			distance += record.distance;
			kcal += record.kcal;
			kcalRest += record.kcalRest;
		}
		...
		public TrackedDataRecord clone()
		{
			...
			sum.intervalStartDateTime = intervalStartDateTime;
			sum.timeZoneSeconds = timeZoneSeconds;
			sum.measurementInterval = measurementInterval;
			sum.steps = steps;
			sum.active = active;
			sum.distance = distance;
			sum.kcal = kcal;
			sum.kcalRest = kcalRest;
			sum.unknown = unknown;
	...
	private DateTime epochToUtcDate(int seconds)
	{
		...
		dateTime = DateTime.forInstant(seconds * 1000L, TimeZone.getTimeZone("UTC"));
	...
	private boolean processTrackedActivityRecord(TrackerProto.TrackRecord record)
	{
		...
		/*
		 * System.out.print(record.getRecordId()+" "+epochToUtcDate(record.getTime())+" zone "+record.getTimeZone()+" steps "+record.getSteps()+" act "+record.
		 * getActive()+" dist "+record.getDistance()+" kcal "+ record.getKcal()+" kcalrest "+record.getKcalRest()+" u1 "+record.getUnknown1()); if
		 * (record.hasSleepMode()) { System.out.print(" sleepmode "+record.getSleepMode()); } if (record.hasSleepTime()) {
		 * System.out.print(" sleeptime "+record.getSleepTime()); } System.out.println();
		 */
		return error;
	}
	 ...
	 * @param dateTime
	 *            Date time
	 * @return Fitness point counter value. If the datetime is prior to the values in the list, 0 is returned.
	 */
	...
	private boolean processFitnessRecord(TrackerProto.FitnessRecord record)
	{
		...
		fitnessRecord.dateTime = epochToUtcDate(record.getTime());
	...
	/**
	 * Clears all the data stored. Should be called prior to parsing a new set of activity track files.
	 */
	public void clear()
	{
		heartRates.clear();
	...
	/**
	 * Convert the data to hourly data. Deduce sleeping periods, if any Sleep calculation: A sleep period consists of mode SLEEPMODE_FIRSTSLEEP followed by
	 * SLEEPMODE_SLEEP. Sum sleep duration during the period
	 */
	public void convertToHourly()
	{
		...
		while (it.hasNext())
		{
					...
					(hourlyRecord == null))
			{
				...
				{
					currentFitnessPoints = getFitnessPointCounter(record.intervalStartDateTime);
					...
					trackedDataPerHour.add(hourlyRecord);
	...
	/**
	 * Returns the tracked activity as a table in a String. Values per hour are shown.
	 *
	...
	public String trackedActivityToString()
	{
		...
		it = trackedDataPerHour.iterator();
	...
	/**
	 * Returns the tracked heart rates as a table in a String. All values are shown.
	 *
	...
	/**
	 * Returns the sleeping periods as a table in a String. All values are shown.
	 *
